...
| **Suggestion engine** (`planning/suggestion.py` + `planning/plan_builder.py`, Milestone 4) | Extends the selection engine with AI-suggested recipes: `build_weekly_plan` reserves `ai_suggestions_per_plan` slots (never drawn from the library) and fills each via `plan_builder._generate_suggestion_meal`, which calls `generate_search_candidates` (a pluggable `SearchProvider` - see "Online recipe search" below) for up to 3 candidates, falling back to a single `generate_combination_suggestion` (blends two randomly-picked liked recipes' ingredients/steps into one free-text brief, extracted through the same pipeline as a manual submission) if the search provider has nothing. **Every** extracted candidate is written into the recipe library immediately, liked by default - see design.md's updated Recipe concept - one is picked at random to fill the slot, and the full committed set (including the pick) is recorded in `MealPlanStore`'s `plan_meal_candidates` table via `MealSpec.candidate_recipe_ids`, so the plan-review UI can later offer "the same N already-fetched dishes" for that slot without a second Spoonacular call. Reroll: whole-plan (`replace_meals` - runs the same generation again, clearing every slot's candidate rows first), single-meal (`generate_single_replacement` - prefers an unused liked library recipe, only generates a fresh 3-candidate batch if the library has nothing left), controlled reroll (`list_controlled_reroll_candidates` - up to 10 unused liked library recipes to pick from directly; deliberately **not** 10 fresh LLM generations, to avoid the latency/cost of extracting and mostly discarding nine of them per request), and - new, suggestion-slot-only - picking among a slot's own already-fetched siblings (`MealPlanStore.get_candidates` + `POST .../choose-suggestion`) with no new fetch and no reroll to a brand-new fourth candidate, per `docs/spoonacular_plan.md`'s "Weekly meals" spec. All reroll/choose actions are blocked once a plan is finalized. A suggestion source that can't produce anything (too few liked recipes, an unconfigured search provider, an LLM hiccup) just contributes nothing to that slot rather than failing the whole request. |
...
| Online recipe search | `planning/suggestion.SpoonacularSearchProvider`, implementing the `SearchProvider` protocol (`search_many(food_filter, count) -> list[str]`, `get_substitutes(ingredient_name) -> Optional[IngredientSubstitutes]`) against the [Spoonacular](https://spoonacular.com/food-api) recipe API; falls back to `NullSearchProvider` (always returns nothing) when no API key is configured | Chosen over general web-search APIs (Google/Bing/Brave - see below) because it's recipe-specific: `complexSearch` finds candidates, `informationBulk` returns their real ingredients/instructions directly, no page-scraping needed. At the household's actual usage (1-3 AI suggestions/week, 3 candidates each), Spoonacular's free tier (~150 requests/day) has enormous headroom - see `milestones.md`'s M4 entry for the fuller provider comparison this decision was based on. Its own classification/nutrition fields are deliberately unused - results are reformatted as free text and passed through the same extraction service every other suggestion source uses, so classification/nutrition stays consistent (and LLM-derived) regardless of where a suggestion came from. Configured via either `LITTLE_MEALS_SPOONACULAR_API_KEY` directly or the encrypted-vault path below (see the README's runtime configuration table); `create_app` picks the provider automatically based on whether `Settings.spoonacular_api_key` ends up set. Every `complexSearch` call unconditionally sets `sort=random`, `type=main course`, `instructionsRequired=true`, and `addRecipeNutrition=true` - see `docs/spoonacular_plan.md`: results must be full meals, never a side dish/dessert/etc., never the same top-ranked handful every time, every candidate must have usable steps before it's ever extracted, and nutrition data must be present since the household's filter can constrain on it. `planning/suggestion.build_complex_search_params(food_filter, count=None)` is the single function that turns the household's own Spoonacular query dict (plus the always-on constants above, which win on any overlapping key) into the full parameter dict - both `SpoonacularSearchProvider.search_many` (which adds `apiKey` on top) and the `/settings/recipe-preferences` page's request preview call it, so what the household is shown is exactly what gets sent, never a hand-maintained approximation of it. `SearchProvider`'s `food_filter` parameter is `Optional[dict]` - the household's own `complexSearch` query parameters, built field-by-field on the dedicated `/settings/recipe-preferences` page (`planning/spoonacular_fields.py`'s `FILTER_SECTIONS`/`MACRONUTRIENTS`/`VITAMINS_AND_MINERALS` drive both that page's text/single-select/multi-select fields and `parse_filter_form`'s parsing back into this dict - see `docs/spoonacular_api.md`/`docs/spoonacular_schema.yaml` for the full parameter list) and passed through as-is, never LLM-generated or otherwise interpreted. `search_many(food_filter, count)` is the sole fetch method (the earlier one-result-at-a-time `search` was removed once every caller needed a batch) - always a flat 2 API calls regardless of `count`: one `complexSearch` (with `number=count`) plus one `informationBulk` call for every result's full ingredients/instructions, never one `/recipes/{id}/information` call per candidate. Used both by the standalone `lmeals import-spoonacular` command (`planning/spoonacular_import.py`, `count` = however many recipes it's asked to import) and by `plan_builder._generate_suggestion_meal` (`count` = 3, one batch per AI-suggestion slot - see the "Suggestion engine" row above). The import command's filter is the household's one saved `food_filter` (it hard-errors if none exists yet, rather than falling back to an unfiltered search), defaults `count` to `recipes_per_week` when not given on the CLI, and runs each result through the same extraction pipeline before writing it straight into the recipe library via `RecipeStore.create` - a single recipe failing extraction is skipped, not fatal to the batch, same as every other suggestion source above. `--reset` wipes the library first via `RecipeStore.delete_all` (removes every `.md` file, including ones that fail to parse) - irreversible, so it prints the file count and prompts `y/N` before deleting unless `--yes` is also passed. `import-spoonacular` is designed to run **concurrently** with `lmeals serve` (not just standalone): `RecipeStore` writes are already atomic (tmp file + `os.replace`), and `sqlite3.connect()`'s default 5s busy-timeout absorbs brief read/write overlap on `household.db` between the two processes without extra config - the one real risk is a first-run schema-migration race, handled by `HouseholdPreferencesStore._migrate_legacy_schema`'s per-`ALTER TABLE` error-swallowing. `get_substitutes(ingredient_name)` (`GET /food/ingredients/substitutes`) backs the shopping-list "find a substitute" action (`routes_shopping.py`'s `/api/shopping-list/{list_id}/items/{item_id}/substitutes`, and the mirrored `/shopping/items/{item_id}/substitutes` htmx partial) - looked up by the shopping-list item's plain merged name (there's no Spoonacular ingredient id to look up by), read-only and stateless, never persisted. |
...
Household preferences (recipes-per-week count, recommendation day/time, free-text
food preferences, a Spoonacular search filter, AI suggestions per plan, default
servings — see `design.md`'s "Household preferences" concept) are a single row in
SQLite (`household.db` under the data directory), per the "Other storage" decision
above — app-managed configuration, not something the user is expected to hand-edit,
and there is exactly one of it per installation, not a collection.
...
since "unconfigured" is a normal, expected state, not an error condition.
`/settings` is one form/action for the general fields: saving it writes
recipes-per-week, day/time, AI suggestions, default servings, and
`food_preferences_text` together via `put()`. The Spoonacular search filter lives
on its own dedicated `/settings/recipe-preferences` page instead (see "Online
recipe search" above) - saving it calls `save_food_filter()` separately, so a
partial JSON API update that omits `food_filter` never clobbers it. Both are
exposed as a JSON API (`GET`/`PUT`/`DELETE` on `/api/household-preferences` —
`DELETE` resets to defaults rather than leaving the household unconfigured) and as
server-rendered forms, all in `api/`.
