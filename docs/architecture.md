...
| **Recipe store** | The recipe library as a directory of Markdown files, one file per recipe (YAML frontmatter for structured fields — cook time, classification, nutrition, ingredients — plus a Markdown body for the ordered steps). Directly readable and editable by the user with any text editor; the backend treats this directory as the source of truth rather than caching it in a database. `RecipeStore.list()` skips a file it can't parse (broken/missing frontmatter, or a recipe hand-pasted in with no frontmatter at all), logging a warning rather than raising. |
| **Data store** | Persists everything that isn't a recipe: household preferences, meal plans, suggestions, shopping lists (which reference recipes by filename/id in the recipe store), and the ingredient pantry/never-buy catalog (`IngredientCatalogStore`, Milestone 18 - a household-wide flag per ingredient name, not per-recipe). Shared by every device in the household — see "Remote access" below — not partitioned per user. |

## Technical decisions

| Decision | Choice | Rationale |
|---|---|---|
| Backend language/framework | Python, FastAPI | Consistent with the rest of the `little-projects` ecosystem (Python + Bazel + wheel packaging, per the [build policy](../../docs/policies/build_policy.md)); FastAPI's typed request/response models are a natural fit for the structured recipe schema. |
...
An ingredient's quantity and unit are separate frontmatter fields
(`quantity: 2` / `unit: piece`), matching the recipe editor's two input boxes.
Some recipe files predate that split and instead have the whole thing crammed
into `unit` (e.g. `unit: 2 piece`, `quantity` unset) — `RecipeStore` tolerantly
splits a leading number back out of `unit` into `quantity` on every read (see
`_normalize_ingredients` in `recipe_store.py`), so those files behave
correctly (in particular, shopping-list merging - see below) without a
one-time migration, and self-heal to the split form the next time they're
...
None — every dependency is pip-installable and resolved hermetically by
Bazel (see "Build" below). Milestone 16 removed the last one, Ollama (a
host prerequisite that couldn't be isolated by Bazel), along with the LLM
extraction feature it backed.

### Preflight dependency check

`src/little_meals/preflight.py` implements the build policy's "missing system
dependencies" requirements: a single "core tools" tier (`python3`, `pipx`),
with every missing dependency reported together in one pass with a
copy-pasteable install command. Exposed as `bazel run //:preflight` and
`lmeals preflight`, and run automatically as the first step of `bazel run
//:install`.
...
|---|---|
| `config.py` | Runtime `Settings` (data dir), all env-overridable. |
| `models.py` | Pydantic `Recipe`/`Ingredient`/`Nutrition`, `HouseholdPreferences`, `MealPlan`/`PlanMeal`, and `ShoppingList`/`ShoppingListItem`. |
| `store/` | The Markdown+YAML-frontmatter recipe store (see "Recipe storage format" above), the SQLite-backed `HouseholdPreferencesStore` (see "Household preferences storage" above), the SQLite-backed `MealPlanStore` (plans + their meals), the SQLite-backed `ShoppingListStore` (one list per plan, keyed by `plan_id`), the SQLite-backed `NotificationStore` (the single "new plan ready" flag), and the SQLite-backed `IngredientCatalogStore` (Milestone 18 - one row per ingredient name, `pantry`/`never_buy` flags, keyed by name not by recipe) - all five app-managed for the same reason as household preferences. |
| `planning/` | `selection.py` (Milestone 3) - the library-only selection engine, a pure function over recipes, no I/O. `plan_builder.py` - `build_weekly_plan`, `generate_single_replacement`, `list_controlled_reroll_candidates`, and `build_meal_specs` (the shared "generate a fresh plan's meals" call every generation path - initial, whole-plan reroll, and the Milestone 7 scheduler - uses), all drawing only from the library (Milestone 4's online-search/combination suggestion generators were removed - see `milestones.md`'s M10 entry). `shopping_list.py` (Milestone 5; Milestone 18) - `build_shopping_list_items` (ingredient merge/scale/never-buy-filter logic) and `group_shopping_list_items` (UI section grouping). `ingredient_grouping.py` (Milestone 18) - `categorize_ingredients` (recipe detail/cook-along display, omits empty groups) and `group_ingredient_names_by_category`/`category_for` (the `/settings/ingredients` management page, always returns all 3 groups). `glob_match.py` (Milestone 18) - `matches_glob`, a shared case-insensitive fnmatch wrapper used by both the ingredient catalog page's search box and `recipe_filters.py`'s cookbook glob search. `recipe_filters.py` (Milestone 17; glob search Milestone 18) - `filter_recipes`/`sort_recipes` plus `GlobScope` (name/ingredients/steps/all) for the cookbook's glob search field. |
